...
public class CommitterEventHandler extends AbstractService
    ...
    protected void serviceStart() throws Exception {
        ...
        ThreadFactory tf = tfBuilder.build(); // 床ThreadFactory
        launcherPool = new HadoopThreadPoolExecutor(5, 5, 1,
                TimeUnit.HOURS, new LinkedBlockingQueue<Runnable>(), tf);  // 创建线程池
        // 创建线程
        eventHandlingThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (!stopped.get() && !Thread.currentThread().isInterrupted()) {
                    try {
                        event = eventQueue.take(); // 从队列中获取一个事件
                    ...
                    // using a thread pool
                    launcherPool.execute(new EventProcessor(event)); // 拿到一个事件就另起一个CommitterEventHandler.EventProcessor线程
                }
            }
        });
        eventHandlingThread.setName("CommitterEvent Handler");
        eventHandlingThread.start(); // 开始执行线程eventHandlingThread的run()函数
        super.serviceStart();
    }
    ...
    private class EventProcessor implements Runnable {
        ...
        public void run() {
            ...
            switch (event.getType()) {
                case JOB_SETUP: // 现在要处理的JOB_SETUP事件
        ...
        protected void handleJobSetup(CommitterJobSetupEvent event) {
            try {
                committer.setupJob(event.getJobContext()); // == OutputCommitter.setupJob(),FileOutputCommitter.setupJob()，设置好本作业所有输出文件所在的目录

                context.getEventHandler().handle(
                        new JobSetupCompletedEvent(event.getJobID())); // 向JobImpl发送JOB_SETUP_COMPLETED事件
            } catch (Exception e) {
